...
const Services = () => {
  return (
    <div id="services" className="dark:bg-[#2a2e2e] bg-white py-16 px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {services.map(({ id, icon, title, description, tools }, i) => (
            <div
              ...
              className="dark:bg-[#353A3A] bg-gray-50 border border-transparent rounded-xl shadow-md p-8 flex flex-col items-center text-center hover:border-[#eab949] hover:shadow-[0_0_24px_rgba(234,185,73,0.2)] hover:-translate-y-1 transition-all duration-300"
            >
              {icon}
              <h3 className="text-xl font-bold dark:text-white text-[#1a1a1a] mb-3">{title}</h3>
              <p className="dark:text-gray-400 text-gray-600 mb-5 text-sm leading-relaxed">
                {description}
              </p>
              <div className="flex flex-wrap justify-center gap-2 mt-auto">
                {tools.map((tool) => (
                  <span
                    key={tool}
                    className="dark:bg-[#1e2222] bg-[#353A3A] text-white text-xs px-3 py-1 rounded-full"
                  >
                    {tool}
                  </span>
                ))}
